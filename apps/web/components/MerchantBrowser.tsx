...
// The "OP trust min" filter and the live derived-tier fetch that backed it
// were removed with the rest of the trust-tier display — see
// lib/display-policy.ts for why, and for the condition to restore them.
...
export function MerchantBrowser({ merchants, categories, rails }: Props) {
  ...
  }, [merchants, q, rail, category, callable, usdc]);

  const filterActive = Boolean(q || rail || category || callable || usdc);
